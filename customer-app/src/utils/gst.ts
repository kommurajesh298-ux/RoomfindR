...
const resolveMonthlyRent = (booking: BookingWithDetails): number => {
  ...
  const roomPrice = booking.propertyDetails?.rooms?.[booking.roomId]?.price;
  if (typeof roomPrice === 'number' && Number.isFinite(roomPrice) && roomPrice > 0) {
    return roundCurrency(roomPrice);
  }
