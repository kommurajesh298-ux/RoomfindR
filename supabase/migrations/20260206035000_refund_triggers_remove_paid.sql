BEGIN;
-- Remove invalid 'paid' enum usage for payment_status_enum in refund automation
...
END;
$$;
-- Backfill: initiate refunds for already rejected/cancelled bookings with completed/success/authorized payments and no refund record
DO $$
DECLARE
    supabase_url TEXT;
    service_key TEXT;
...
BEGIN
    ...
    FOR rec IN
        ...
        WHERE lower(b.status::text) IN ('rejected','cancelled','cancelled_by_customer','cancelled-by-customer')
          AND lower(p.status::text) IN ('completed','success','authorized')
          AND NOT EXISTS (
