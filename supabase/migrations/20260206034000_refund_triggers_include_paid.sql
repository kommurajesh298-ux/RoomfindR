BEGIN;
-- Ensure refunds trigger covers payments marked as paid
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE booking_id = NEW.id
          AND status IN ('completed','success','authorized','paid')
    ) THEN
        RETURN NEW;
    END IF;
...
BEGIN
    ...
    IF lower(NEW.status::text) NOT IN ('completed','success','authorized','paid') THEN
        RETURN NEW;
    END IF;

    IF lower(OLD.status::text) IN ('completed','success','authorized','paid') THEN
        RETURN NEW;
    END IF;
...
END;
$$;
-- Backfill: initiate refunds for already rejected/cancelled bookings with paid payments and no refund record
DO $$
DECLARE
    supabase_url TEXT;
    service_key TEXT;
...
BEGIN
    ...
    IF supabase_url IS NULL OR service_key IS NULL THEN
        RAISE NOTICE 'Missing supabase_url or service key for refund backfill';
        RETURN;
    END IF;
    ...
    FOR rec IN
        ...
        WHERE lower(b.status::text) IN ('rejected','cancelled','cancelled_by_customer','cancelled-by-customer')
          AND lower(p.status::text) IN ('completed','success','authorized','paid')
          AND NOT EXISTS (
            SELECT 1 FROM refunds r
            WHERE r.booking_id = b.id
              AND r.status IN ('PENDING','PROCESSING','SUCCESS','PROCESSED')
          )
    LOOP
        PERFORM net.http_post(
            url := supabase_url || '/functions/v1/cashfree-refund',
