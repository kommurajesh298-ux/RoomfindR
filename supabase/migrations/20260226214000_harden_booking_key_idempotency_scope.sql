...
DECLARE
    ...
    v_existing_is_creation_pending BOOLEAN;
BEGIN
    ...
    -- Idempotency key can only dedupe booking-creation retries, never attach to active stays.
    IF v_booking_key IS NOT NULL THEN
        ...
        IF v_existing_id IS NOT NULL THEN
            ...
            v_existing_is_creation_pending :=
                public.normalize_booking_state_token(v_existing_status) IN ('pending', 'payment-pending', 'requested')
                OR public.normalize_booking_state_token(v_existing_stay_status) IN ('pending', 'payment-pending', 'requested')
                OR public.normalize_booking_state_token(v_existing_booking_status) IN ('pending', 'requested')
                OR public.normalize_booking_state_token(v_existing_continue_status) IN ('pending', 'requested');

            IF NOT v_existing_closed AND v_existing_is_creation_pending THEN
                RETURN jsonb_build_object(
                    'success', true,
                    'booking_id', v_existing_id,
                    'idempotent', true
                );
            ELSIF NOT v_existing_closed THEN
                RAISE EXCEPTION
                    'ACTIVE_PG_BOOKING_EXISTS: You already have an active booking. Please vacate your current PG before booking another one.'
                    USING ERRCODE = 'P0001',
                          DETAIL = format('conflicting_booking_id=%s', v_existing_id);
            END IF;
