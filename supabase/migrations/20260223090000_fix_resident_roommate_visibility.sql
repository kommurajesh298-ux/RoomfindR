BEGIN;
-- Ensure customer roommate visibility is consistent with real in-house residents.
-- Keep function signature unchanged to avoid client breakage.
...
BEGIN
  ...
  SELECT
    ...
    b.customer_phone,
    CASE
      WHEN lower(replace(coalesce(b.stay_status::text, ''), '_', '-')) IN (
        'checked-in',
        'active',
        'ongoing',
        'vacate-requested'
...
-- Ensure PostgREST exposes the latest function body immediately.
