...
WITH paid_advance AS (
  SELECT DISTINCT p.booking_id
  FROM public.payments p
  WHERE lower(COALESCE(p.payment_type, '')) = 'advance'
    AND lower(COALESCE(p.payment_status, p.status, '')) IN (
      'paid',
      'completed',
      'success',
      'held',
...
SET
  ...
  settlement_status = CASE
    WHEN lower(COALESCE(b.settlement_status, 'pending')) IN ('pending', 'queued', 'requested', 'payment_pending')
      THEN 'eligible'
    ELSE b.settlement_status
  END,
  updated_at = timezone('utc', now())
FROM paid_advance pa
WHERE pa.booking_id = b.id
  AND COALESCE(b.owner_accept_status, FALSE) = TRUE
  AND (
    lower(COALESCE(b.status::TEXT, '')) IN ('pending', 'requested', 'payment_pending', 'paid')
    OR lower(COALESCE(b.booking_status, '')) IN ('pending', 'requested', 'payment_pending', 'paid')
  );
COMMIT;
