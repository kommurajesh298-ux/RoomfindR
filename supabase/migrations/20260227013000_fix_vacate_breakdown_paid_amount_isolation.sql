...
BEGIN
    ...
    WHERE booking_id = p_booking_id
      ...
      AND lower(COALESCE(payment_status, status, '')) IN (
          'paid',
          'completed',
          'success',
          'authorized',
    ...
    IF v_last_rent_payment.id IS NOT NULL THEN
        v_total_rent_paid := GREATEST(0, COALESCE(v_last_rent_payment.amount, 0));
    ELSE
        v_is_full_cycle_prepaid := lower(COALESCE(v_booking.payment_type, '')) = 'full'
            AND (
                lower(COALESCE(v_booking.payment_status, '')) IN (
                    'paid',
                    'completed',
                    'success',
                    'authorized',
        ...
        IF v_is_full_cycle_prepaid THEN
            -- Full-plan bookings can prepay the current cycle without a rent payment row.
            v_total_rent_paid := GREATEST(0, COALESCE(v_booking.monthly_rent, 0));
        ELSE
            -- Advance-only bookings must not be treated as rent-paid.
            v_total_rent_paid := 0;
        END IF;
    END IF;
