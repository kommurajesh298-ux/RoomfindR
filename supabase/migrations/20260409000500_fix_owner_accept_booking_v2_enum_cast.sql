...
BEGIN
  ...
  v_charge_confirmed :=
    lower(COALESCE(v_booking.payment_status::TEXT, '')) = ANY(v_paid_statuses)
  ...
  IF NOT v_charge_confirmed AND v_has_payments_table THEN
    ...
    IF v_has_payment_status_col OR v_has_status_col THEN
      IF v_has_payment_status_col AND v_has_status_col THEN
        v_payment_state_expr := 'coalesce(p.payment_status::text, p.status, '''')';
      ELSIF v_has_payment_status_col THEN
        v_payment_state_expr := 'coalesce(p.payment_status::text, '''')';
      ELSE
