...
const TOKEN_KEY = 'roomfindr_owner_push_token';
...
const attachListeners = () => {
  ...
  PushNotifications.addListener('registration', async (token) => {
    try {
      ...
      if (import.meta.env.DEV) {
        console.error('Owner push registration error:', error);
      }
    }
  });
  ...
  PushNotifications.addListener('registrationError', (error) => {
    if (import.meta.env.DEV) {
      console.error('Owner push registration failed:', error);
    }
  });
...
export const pushService = {
  ...
  register: async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        console.warn('Owner native push registration skipped:', error);
      }
    }
  },
