...
const PaymentStatusPage: React.FC = () => {
  ...
  const scheduleRedirect = (url: string, nextMessage: string) => {
  ...
  useEffect(() => {
    return () => {
      if (redirectRef.current.delay) window.clearTimeout(redirectRef.current.delay);
      if (redirectRef.current.fallback) window.clearTimeout(redirectRef.current.fallback);
    };
  }, []);

  useEffect(() => {
    // Poll booking/payment status until webhook updates the DB.
    if (!bookingIdParam && !orderIdParam) {
  ...
  }, [appQuery, bookingIdParam, navigate, orderIdParam]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-slate-50 via-white to-orange-50 p-6">
      <div className="bg-white rounded-[2.5rem] shadow-2xl border border-orange-100 p-10 w-full max-w-lg text-center">
        ...
        <p className="text-gray-600 font-medium mt-2">{error || message}</p>
