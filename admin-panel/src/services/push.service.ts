...
const TOKEN_KEY = 'roomfindr_admin_push_token';
const PENDING_PUSH_ROUTE_KEY = 'roomfindr_admin_pending_push_route';
const PUSH_OPEN_EVENT = 'roomfindr-admin:push-open';
const DEFAULT_CHANNEL_ID = 'default';
const DEVICE_TOKENS_SCHEMA_MISSING = 'PGRST205';
const DEFAULT_CHANNEL: Channel = {
  id: DEFAULT_CHANNEL_ID,
  name: 'RoomFindR Admin Alerts',
  description: 'Platform, booking, payment, refund, and system updates',
  importance: 5,
  visibility: 1,
};
...
const attachListeners = () => {
  ...
  PushNotifications.addListener('pushNotificationReceived', () => {
    // Foreground state relies on Supabase Realtime for UI refresh.
  });
...
export const pushService = {
  ...
  register: async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        console.warn('Admin native push registration skipped:', error);
      }
    }
  },
